		...
		while( bytesRead > 0 )
		{
			...
			if( [currentParseRun.errorsFatal count] > 0 )
			{
				...
				else
				{
					...
					{
						SVGKitLogWarn(@"[%@] ... FATAL ERRROR in SVG parse: %@", [self class], error );
					}
				}
...
static void startElementSAX (void *ctx, const xmlChar *localname, const xmlChar *prefix,
	...
	SVGKitLogWarn(@"[%@] DEBUG_VERBOSE: <%@%@> (namespace URL:%@), attributes: %i", [NSctx class], [NSString stringWithFormat:@"%@:",stringPrefix], name, stringURI, nb_attributes );
#endif
#endif
...
{
	SVGKitLogWarn(@"Error: unparsed entity Decl, name: %@, public ID: %@, system ID: %@, notationName: %@", NSStringFromLibxmlString(name),
